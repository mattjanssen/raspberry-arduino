...
def ColorFromPalette(pal, index, brightness=255, blendType=None):
    if isinstance(pal, TProgmemRGBPalette16):
        ...
        entry = CRGB(pal[0] + hi4) # Removed FL_PGM_READ_DWORD_NEAR
        ...
        if blend:
            if hi4 == 15:
                entry = CRGB(pal[0]) # Removed 
            else:
                entry = CRGB(pal[1 + hi4]) # Removed FL_PGM_READ_DWORD_NEAR
